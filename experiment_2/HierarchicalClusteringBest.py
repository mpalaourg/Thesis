...
if __name__ == "__main__":
    ...
    modelAverage  = AgglomerativeClustering(n_clusters=7, affinity="precomputed", linkage='average').fit(distance)
    labels = modelAverage.fit_predict(distance)
    silhouette_score = metrics.silhouette_score(distance, labels, metric="precomputed")
    print(f"silhouette = {silhouette_score} for average and 7 clusters.")
