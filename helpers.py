...
def normData():
    ...
    for f in os.listdir(current_directory+'\\csvFiles_PC'):
        if not os.path.exists(current_directory+'\\csvFiles\\'+f):
            shutil.copy(current_directory+'\\csvFiles_PC\\'+f, current_directory+'\\csvFiles')
    
    for f in os.listdir(current_directory+'\\csvFiles_rasp'):
        if not os.path.exists(current_directory+'\\csvFiles\\'+f):
            print(f)
            shutil.copy(current_directory+'\\csvFiles_rasp\\'+f, current_directory+'\\csvFiles')
  
    csvNorm_directory = current_directory + '\\csvFiles_norm'
