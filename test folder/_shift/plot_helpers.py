...
#import metrics
#import sklarn.metrics as metrics 
...
def plot_dSOC(y_true, y_est):#, sampleNumber):
    ...
    AE = abs(real_dSOC - est_dSOC)
    ax2.scatter(range(len(real_dSOC)), AE, label = 'Absolute Error', color = 'black', marker = 'o')    
    ax2.set_title(f"Error of dSOC/dt Estimation")
    ax2.legend()
    ax2.set(ylabel = 'AE', xlabel = 'Test Case')
